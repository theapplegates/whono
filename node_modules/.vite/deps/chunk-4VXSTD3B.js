import {
  Icon_default
} from "./chunk-A2GX7KI6.js";
import {
  check_target,
  hmr,
  legacy_api,
  rest_props,
  spread_props
} from "./chunk-GXYAJ2SO.js";
import {
  append,
  comment
} from "./chunk-WV5M5R6D.js";
import {
  FILENAME,
  HMR,
  add_svelte_meta,
  first_child,
  pop,
  push
} from "./chunk-VOUYE5FZ.js";

// node_modules/@lucide/svelte/dist/icons/fast-forward.svelte
Fast_forward[FILENAME] = "node_modules/@lucide/svelte/dist/icons/fast-forward.svelte";
var rest_excludes = /* @__PURE__ */ new Set(["$$slots", "$$events", "$$legacy"]);
function Fast_forward($$anchor, $$props) {
  check_target(new.target);
  push($$props, true, Fast_forward);
  let props = rest_props($$props, rest_excludes, "props");
  const iconNode = [
    [
      "path",
      {
        "d": "M12 6a2 2 0 0 1 3.414-1.414l6 6a2 2 0 0 1 0 2.828l-6 6A2 2 0 0 1 12 18z"
      }
    ],
    [
      "path",
      {
        "d": "M2 6a2 2 0 0 1 3.414-1.414l6 6a2 2 0 0 1 0 2.828l-6 6A2 2 0 0 1 2 18z"
      }
    ]
  ];
  var $$exports = { ...legacy_api() };
  var fragment = comment();
  var node = first_child(fragment);
  add_svelte_meta(
    () => Icon_default(node, spread_props({ name: "fast-forward" }, () => props, {
      get iconNode() {
        return iconNode;
      }
    })),
    "component",
    Fast_forward,
    23,
    0,
    { componentTag: "Icon" }
  );
  append($$anchor, fragment);
  return pop($$exports);
}
if (import.meta.hot) {
  Fast_forward = hmr(Fast_forward);
  import.meta.hot.accept((module) => {
    Fast_forward[HMR].update(module.default);
  });
}
var fast_forward_default = Fast_forward;

export {
  fast_forward_default
};
//# sourceMappingURL=chunk-4VXSTD3B.js.map
