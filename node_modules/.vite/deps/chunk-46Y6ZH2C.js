import {
  Icon_default
} from "./chunk-A2GX7KI6.js";
import {
  check_target,
  hmr,
  legacy_api,
  rest_props,
  spread_props
} from "./chunk-GXYAJ2SO.js";
import {
  append,
  comment
} from "./chunk-WV5M5R6D.js";
import {
  FILENAME,
  HMR,
  add_svelte_meta,
  first_child,
  pop,
  push
} from "./chunk-VOUYE5FZ.js";

// node_modules/@lucide/svelte/dist/icons/file-key.svelte
File_key[FILENAME] = "node_modules/@lucide/svelte/dist/icons/file-key.svelte";
var rest_excludes = /* @__PURE__ */ new Set(["$$slots", "$$events", "$$legacy"]);
function File_key($$anchor, $$props) {
  check_target(new.target);
  push($$props, true, File_key);
  let props = rest_props($$props, rest_excludes, "props");
  const iconNode = [
    ["path", { "d": "M14 2v5a1 1 0 0 0 1 1h5" }],
    ["path", { "d": "M4 12v6" }],
    ["path", { "d": "M4 14h2" }],
    [
      "path",
      {
        "d": "M9.65 22H18a2 2 0 0 0 2-2V8a2.4 2.4 0 0 0-.706-1.706l-3.588-3.588A2.4 2.4 0 0 0 14 2H6a2 2 0 0 0-2 2v4"
      }
    ],
    ["circle", { "cx": "4", "cy": "20", "r": "2" }]
  ];
  var $$exports = { ...legacy_api() };
  var fragment = comment();
  var node = first_child(fragment);
  add_svelte_meta(
    () => Icon_default(node, spread_props({ name: "file-key" }, () => props, {
      get iconNode() {
        return iconNode;
      }
    })),
    "component",
    File_key,
    23,
    0,
    { componentTag: "Icon" }
  );
  append($$anchor, fragment);
  return pop($$exports);
}
if (import.meta.hot) {
  File_key = hmr(File_key);
  import.meta.hot.accept((module) => {
    File_key[HMR].update(module.default);
  });
}
var file_key_default = File_key;

export {
  file_key_default
};
//# sourceMappingURL=chunk-46Y6ZH2C.js.map
