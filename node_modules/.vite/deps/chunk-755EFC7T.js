import {
  Icon_default
} from "./chunk-A2GX7KI6.js";
import {
  check_target,
  hmr,
  legacy_api,
  rest_props,
  spread_props
} from "./chunk-GXYAJ2SO.js";
import {
  append,
  comment
} from "./chunk-WV5M5R6D.js";
import {
  FILENAME,
  HMR,
  add_svelte_meta,
  first_child,
  pop,
  push
} from "./chunk-VOUYE5FZ.js";

// node_modules/@lucide/svelte/dist/icons/folder-key.svelte
Folder_key[FILENAME] = "node_modules/@lucide/svelte/dist/icons/folder-key.svelte";
var rest_excludes = /* @__PURE__ */ new Set(["$$slots", "$$events", "$$legacy"]);
function Folder_key($$anchor, $$props) {
  check_target(new.target);
  push($$props, true, Folder_key);
  let props = rest_props($$props, rest_excludes, "props");
  const iconNode = [
    [
      "path",
      {
        "d": "M13 20H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h3.9a2 2 0 0 1 1.69.9l.81 1.2a2 2 0 0 0 1.67.9H20a2 2 0 0 1 2 2v1.36"
      }
    ],
    ["path", { "d": "M19 12v6" }],
    ["path", { "d": "M19 14h2" }],
    ["circle", { "cx": "19", "cy": "20", "r": "2" }]
  ];
  var $$exports = { ...legacy_api() };
  var fragment = comment();
  var node = first_child(fragment);
  add_svelte_meta(
    () => Icon_default(node, spread_props({ name: "folder-key" }, () => props, {
      get iconNode() {
        return iconNode;
      }
    })),
    "component",
    Folder_key,
    23,
    0,
    { componentTag: "Icon" }
  );
  append($$anchor, fragment);
  return pop($$exports);
}
if (import.meta.hot) {
  Folder_key = hmr(Folder_key);
  import.meta.hot.accept((module) => {
    Folder_key[HMR].update(module.default);
  });
}
var folder_key_default = Folder_key;

export {
  folder_key_default
};
//# sourceMappingURL=chunk-755EFC7T.js.map
