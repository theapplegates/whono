import {
  Icon_default
} from "./chunk-A2GX7KI6.js";
import {
  check_target,
  hmr,
  legacy_api,
  rest_props,
  spread_props
} from "./chunk-GXYAJ2SO.js";
import {
  append,
  comment
} from "./chunk-WV5M5R6D.js";
import {
  FILENAME,
  HMR,
  add_svelte_meta,
  first_child,
  pop,
  push
} from "./chunk-VOUYE5FZ.js";

// node_modules/@lucide/svelte/dist/icons/file-lock.svelte
File_lock[FILENAME] = "node_modules/@lucide/svelte/dist/icons/file-lock.svelte";
var rest_excludes = /* @__PURE__ */ new Set(["$$slots", "$$events", "$$legacy"]);
function File_lock($$anchor, $$props) {
  check_target(new.target);
  push($$props, true, File_lock);
  let props = rest_props($$props, rest_excludes, "props");
  const iconNode = [
    [
      "path",
      {
        "d": "M4 9.8V4a2 2 0 0 1 2-2h8a2.4 2.4 0 0 1 1.706.706l3.588 3.588A2.4 2.4 0 0 1 20 8v12a2 2 0 0 1-2 2h-3"
      }
    ],
    ["path", { "d": "M14 2v5a1 1 0 0 0 1 1h5" }],
    ["path", { "d": "M9 17v-2a2 2 0 0 0-4 0v2" }],
    [
      "rect",
      { "width": "8", "height": "5", "x": "3", "y": "17", "rx": "1" }
    ]
  ];
  var $$exports = { ...legacy_api() };
  var fragment = comment();
  var node = first_child(fragment);
  add_svelte_meta(
    () => Icon_default(node, spread_props({ name: "file-lock" }, () => props, {
      get iconNode() {
        return iconNode;
      }
    })),
    "component",
    File_lock,
    23,
    0,
    { componentTag: "Icon" }
  );
  append($$anchor, fragment);
  return pop($$exports);
}
if (import.meta.hot) {
  File_lock = hmr(File_lock);
  import.meta.hot.accept((module) => {
    File_lock[HMR].update(module.default);
  });
}
var file_lock_default = File_lock;

export {
  file_lock_default
};
//# sourceMappingURL=chunk-TLQDVHTX.js.map
