import {
  Icon_default
} from "./chunk-A2GX7KI6.js";
import {
  check_target,
  hmr,
  legacy_api,
  rest_props,
  spread_props
} from "./chunk-GXYAJ2SO.js";
import {
  append,
  comment
} from "./chunk-WV5M5R6D.js";
import {
  FILENAME,
  HMR,
  add_svelte_meta,
  first_child,
  pop,
  push
} from "./chunk-VOUYE5FZ.js";

// node_modules/@lucide/svelte/dist/icons/file-clock.svelte
File_clock[FILENAME] = "node_modules/@lucide/svelte/dist/icons/file-clock.svelte";
var rest_excludes = /* @__PURE__ */ new Set(["$$slots", "$$events", "$$legacy"]);
function File_clock($$anchor, $$props) {
  check_target(new.target);
  push($$props, true, File_clock);
  let props = rest_props($$props, rest_excludes, "props");
  const iconNode = [
    [
      "path",
      {
        "d": "M16 22h2a2 2 0 0 0 2-2V8a2.4 2.4 0 0 0-.706-1.706l-3.588-3.588A2.4 2.4 0 0 0 14 2H6a2 2 0 0 0-2 2v2.85"
      }
    ],
    ["path", { "d": "M14 2v5a1 1 0 0 0 1 1h5" }],
    ["path", { "d": "M8 14v2.2l1.6 1" }],
    ["circle", { "cx": "8", "cy": "16", "r": "6" }]
  ];
  var $$exports = { ...legacy_api() };
  var fragment = comment();
  var node = first_child(fragment);
  add_svelte_meta(
    () => Icon_default(node, spread_props({ name: "file-clock" }, () => props, {
      get iconNode() {
        return iconNode;
      }
    })),
    "component",
    File_clock,
    23,
    0,
    { componentTag: "Icon" }
  );
  append($$anchor, fragment);
  return pop($$exports);
}
if (import.meta.hot) {
  File_clock = hmr(File_clock);
  import.meta.hot.accept((module) => {
    File_clock[HMR].update(module.default);
  });
}
var file_clock_default = File_clock;

export {
  file_clock_default
};
//# sourceMappingURL=chunk-QEGXIZQL.js.map
