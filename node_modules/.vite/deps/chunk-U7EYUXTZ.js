import {
  Icon_default
} from "./chunk-A2GX7KI6.js";
import {
  check_target,
  hmr,
  legacy_api,
  rest_props,
  spread_props
} from "./chunk-GXYAJ2SO.js";
import {
  append,
  comment
} from "./chunk-WV5M5R6D.js";
import {
  FILENAME,
  HMR,
  add_svelte_meta,
  first_child,
  pop,
  push
} from "./chunk-VOUYE5FZ.js";

// node_modules/@lucide/svelte/dist/icons/monitor-off.svelte
Monitor_off[FILENAME] = "node_modules/@lucide/svelte/dist/icons/monitor-off.svelte";
var rest_excludes = /* @__PURE__ */ new Set(["$$slots", "$$events", "$$legacy"]);
function Monitor_off($$anchor, $$props) {
  check_target(new.target);
  push($$props, true, Monitor_off);
  let props = rest_props($$props, rest_excludes, "props");
  const iconNode = [
    ["path", { "d": "M12 17v4" }],
    [
      "path",
      { "d": "M17 17H4a2 2 0 0 1-2-2V5a2 2 0 0 1 1.184-1.826" }
    ],
    ["path", { "d": "m2 2 20 20" }],
    ["path", { "d": "M8 21h8" }],
    [
      "path",
      { "d": "M8.656 3H20a2 2 0 0 1 2 2v10a2 2 0 0 1-.293 1.042" }
    ]
  ];
  var $$exports = { ...legacy_api() };
  var fragment = comment();
  var node = first_child(fragment);
  add_svelte_meta(
    () => Icon_default(node, spread_props({ name: "monitor-off" }, () => props, {
      get iconNode() {
        return iconNode;
      }
    })),
    "component",
    Monitor_off,
    23,
    0,
    { componentTag: "Icon" }
  );
  append($$anchor, fragment);
  return pop($$exports);
}
if (import.meta.hot) {
  Monitor_off = hmr(Monitor_off);
  import.meta.hot.accept((module) => {
    Monitor_off[HMR].update(module.default);
  });
}
var monitor_off_default = Monitor_off;

export {
  monitor_off_default
};
//# sourceMappingURL=chunk-U7EYUXTZ.js.map
