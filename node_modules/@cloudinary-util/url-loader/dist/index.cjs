...
__export(src_exports, {
  ...
  cloudinaryPluginKeys: () => cloudinaryPluginKeys,
  cloudinaryPluginProps: () => cloudinaryPluginProps,
...
var import_util8 = require("@cloudinary-util/util");
...
var FillBackgroundPlugin = /* @__PURE__ */ plugin({
  ...
  apply: (cldAsset, opts, ctx) => {
    ...
    const width = (0, import_util3.normalizeNumberParameter)(ctx.width);
    const height = (0, import_util3.normalizeNumberParameter)(ctx.height);
  ...
  apply: (cldAsset, opts, ctx) => {
    ...
    if (preserveTransformations) {
      try {
        ...
        const transformations = (0, import_util4.getTransformations)(ctx.src).map(
          (t) => t.join(",")
        );
...
function collectTransformations(collectOptions) {
  ...
  const height = (0, import_util6.normalizeNumberParameter)(collectOptions.height);
  const width = (0, import_util6.normalizeNumberParameter)(collectOptions.width);
  ...
  apply: (cldAsset, opts) => {
    ...
    function applyOverlay({
      ...
      if (hasText) {
        ...
        layerTransformation = `${typeQualifier}_fetch:${(0, import_util7.encodeBase64)(url)}`;
      }
      const primary2 = [];
      const applied = [];
      Object.keys(options).forEach((key) => {
        if (!(0, import_util7.objectHasKey)(primary, key)) return;
      ...
      if (hasText) {
        ...
        if (typeof text3 === "object") {
          ...
          const sortedTextOptions = (0, import_util7.sortByKey)(textOptions, "order");
      ...
      if (hasPosition) {
        Object.entries(position2).forEach(([key, value]) => {
          if (!(0, import_util7.objectHasKey)(position, key)) return;
...
function constructCloudinaryUrl({ options, config = {}, analytics }) {
  ...
  if (typeof options.src === "string" && /^https?:\/\//.test(options.src)) {
    try {
      const parts = (0, import_util8.parseUrl)(options.src);
  ...
  entriesOf(parsedOptions).forEach(([key, value]) => {
    if ((0, import_util8.objectHasKey)(options, key)) return;
    options[key] = value;
  });
...
var import_util9 = require("@cloudinary-util/util");
function getVideoPlayerOptions(options, config) {
  ...
  if (publicId.startsWith("http")) {
    try {
      const parts = (0, import_util9.parseUrl)(src);
