...
export const validateAdminThemeSettings = (
  ...
  if (!settings.site.defaultLocale) {
    pushIssue('site.defaultLocale', 'Default locale cannot be empty');
  } else if (!ADMIN_LOCALE_RE.test(settings.site.defaultLocale)) {
    pushIssue('site.defaultLocale', 'Default locale format is invalid (example: en-US)');
  }
  ...
  if (!settings.site.footer?.copyright) {
    ...
    pushIssue('site.footer.copyright', `Footer copyright line cannot exceed ${ADMIN_FOOTER_COPYRIGHT_MAX_LENGTH} characters`);
  }

  if (typeof settings.site.adminOverview?.publicVisible !== 'boolean') {
    pushIssue('site.adminOverview.publicVisible', 'Admin Overview public toggle must be a boolean');
  }

  if (!settings.site.adminOverview?.hiddenMessage) {
    pushIssue('site.adminOverview.hiddenMessage', 'Admin Overview closed-state message cannot be empty');
  } else if (
    settings.site.adminOverview.hiddenMessage.includes('\n') ||
    settings.site.adminOverview.hiddenMessage.includes('\r')
  ) {
    pushIssue('site.adminOverview.hiddenMessage', 'Admin Overview closed-state message must be single-line text');
    ...
    pushIssue(
      ...
      `Admin Overview closed-state message cannot exceed ${ADMIN_OVERVIEW_HIDDEN_MESSAGE_MAX_LENGTH} characters`
    );
  }
  ...
  if (
    ...
    pushIssue('site.socialLinks.github', 'GitHub link must be https://github.com/... ');
  }
  if (
    settings.site.socialLinks?.x &&
    !isAdminAllowedHttpsUrl(settings.site.socialLinks.x, ADMIN_X_HOSTS)
  ) {
    pushIssue('site.socialLinks.x', 'X / Twitter link must be https://x.com/... or https://twitter.com/... ');
  }
  if (
  ...
  ADMIN_SOCIAL_PRESET_IDS.forEach((id) => {
    ...
    if (orderIssue === 'duplicate') {
      pushIssue(`site.socialLinks.presetOrder.${id}`, `Social link order must not duplicate: ${presetOrder[id]}`);
    }
  });
  ...
  customLinks.forEach((item, index) => {
    ...
    if (!item.id) {
      ...
      if (item.id.includes('\n') || item.id.includes('\r')) {
        pushIssue(`${basePath}.id`, `Custom link #${index + 1} ID must be single-line text`);
      }
      if (seenCustomIds.has(item.id)) {
        pushIssue(`${basePath}.id`, `Custom link ID must not duplicate: ${item.id}`);
      }
    ...
    if (!item.label) {
      ...
      pushIssue(`${basePath}.label`, `Custom link #${index + 1} display name must be single-line text`);
    }
    ...
    if (!isAdminSocialIconKey(item.iconKey)) {
      pushIssue(`${basePath}.iconKey`, `Custom link #${index + 1} icon must be chosen from the allowlist`);
    }
    const orderIssue = customOrderIssues.get(index);
    if (orderIssue === 'range') {
      pushIssue(
        `${basePath}.order`,
        `Custom link #${index + 1} order must be an integer between ${ADMIN_SOCIAL_ORDER_MIN} and ${ADMIN_SOCIAL_ORDER_MAX}`
      );
    } else if (orderIssue === 'duplicate') {
      pushIssue(`${basePath}.order`, `Social link order must not duplicate: ${item.order}`);
    }
  ...
  if (!settings.home.introLead) {
    pushIssue('home.introLead', 'Home intro lead copy cannot be empty');
  } else if (settings.home.introLead.length > ADMIN_HOME_INTRO_MAX_LENGTH) {
    pushIssue('home.introLead', `Home intro lead copy cannot exceed ${ADMIN_HOME_INTRO_MAX_LENGTH} characters`);
  }

  if (typeof settings.home.showIntroLead !== 'boolean') {
    pushIssue('home.showIntroLead', 'Home intro lead visibility toggle must be a boolean');
  }

  if (!settings.home.introMore) {
    pushIssue('home.introMore', 'Home intro follow-up copy cannot be empty');
  } else if (settings.home.introMore.length > ADMIN_HOME_INTRO_MAX_LENGTH) {
    pushIssue('home.introMore', `Home intro follow-up copy cannot exceed ${ADMIN_HOME_INTRO_MAX_LENGTH} characters`);
  }

  if (typeof settings.home.showIntroMore !== 'boolean') {
    pushIssue('home.showIntroMore', 'Home intro follow-up visibility toggle must be a boolean');
  }

  if (!Array.isArray(settings.home.introMoreLinks)) {
    pushIssue('home.introMoreLinks', 'Home intro follow-up links must be an array');
  } else if (
    settings.home.introMoreLinks.length < 1 ||
    settings.home.introMoreLinks.length > ADMIN_HOME_INTRO_LINK_LIMIT
  ) {
    pushIssue('home.introMoreLinks', `Home intro follow-up links must select 1-${ADMIN_HOME_INTRO_LINK_LIMIT} entries`);
  } else {
    ...
    settings.home.introMoreLinks.forEach((linkKey, index) => {
      if (!isAdminHomeIntroLinkKey(linkKey)) {
        pushIssue(`home.introMoreLinks[${index}]`, `Home intro follow-up link #${index + 1} is invalid: ${String(linkKey)}`);
        return;
      }
      if (seenHomeIntroLinks.has(linkKey)) {
        pushIssue(`home.introMoreLinks[${index}]`, `Home intro follow-up links must not duplicate: ${linkKey}`);
        return;
      }
  ...
  if (!ADMIN_HERO_PRESET_SET.has(settings.home.heroPresetId)) {
    pushIssue('home.heroPresetId', 'Hero display mode must be default or none');
  }
  ...
  if (
    ...
    pushIssue(
      'home.heroImageSrc',
      'Hero image URL must be under src/assets/**, public/** (or a path starting with /), or an https:// image URL'
    );
    ...
    if (localFilePath && options.localFileExists && !options.localFileExists(localFilePath)) {
      pushIssue('home.heroImageSrc', `Hero image points to a local file that does not exist: ${localFilePath}`);
    }
  }

  if (!settings.home.heroImageAlt) {
    pushIssue('home.heroImageAlt', 'Hero image alt text cannot be empty');
  } else if (
    ...
    pushIssue('home.heroImageAlt', `Hero image alt text cannot exceed ${ADMIN_HERO_IMAGE_ALT_MAX_LENGTH} characters`);
  }
  ...
  pageTitleMap.forEach(([title, label, path]) => {
    ...
    if (typeof title !== 'string') {
      pushIssue(path, `${label} must be a string or empty`);
      return;
    }
    if (title.includes('\n') || title.includes('\r')) {
  ...
  if (settings.page.bits?.defaultAuthor?.avatar) {
    if (normalizeAdminBitsAvatarPath(settings.page.bits.defaultAuthor.avatar) === undefined) {
      pushIssue(
        ...
        'Bits default avatar must be a relative image path (e.g. author/avatar.webp); must not include public/, must not start with /, and must not contain URL, .., ?, #'
      );
    }
  }

  if (typeof settings.ui?.articleMeta?.showDate !== 'boolean') {
    pushIssue('ui.articleMeta.showDate', 'Article meta "show date" must be a boolean');
  }

  if (typeof settings.ui?.articleMeta?.dateLabel !== 'string') {
    pushIssue('ui.articleMeta.dateLabel', 'Article meta "date prefix" must be a string');
  } else if (
    settings.ui.articleMeta.dateLabel.includes('\n') ||
    settings.ui.articleMeta.dateLabel.includes('\r')
  ) {
    pushIssue('ui.articleMeta.dateLabel', 'Article meta "date prefix" must be single-line text');
  } else if (settings.ui.articleMeta.dateLabel.length > ADMIN_ARTICLE_META_DATE_LABEL_MAX_LENGTH) {
    pushIssue(
      'ui.articleMeta.dateLabel',
      `Article meta "date prefix" cannot exceed ${ADMIN_ARTICLE_META_DATE_LABEL_MAX_LENGTH} characters`
    );
  }

  if (typeof settings.ui?.articleMeta?.showTags !== 'boolean') {
    pushIssue('ui.articleMeta.showTags', 'Article meta "show tags" must be a boolean');
  }

  if (typeof settings.ui?.articleMeta?.showWordCount !== 'boolean') {
    pushIssue('ui.articleMeta.showWordCount', 'Article meta "show word count" must be a boolean');
  }

  if (typeof settings.ui?.articleMeta?.showReadingTime !== 'boolean') {
    pushIssue('ui.articleMeta.showReadingTime', 'Article meta "show reading time" must be a boolean');
  }

  if (typeof settings.ui?.sidebarActions?.showRssLink !== 'boolean') {
    pushIssue('ui.sidebarActions.showRssLink', 'Sidebar "show RSS entry" must be a boolean');
  }

  if (typeof settings.ui?.sidebarActions?.showThemeToggle !== 'boolean') {
    pushIssue('ui.sidebarActions.showThemeToggle', 'Sidebar "show theme toggle" must be a boolean');
  }

  if (typeof settings.ui?.sidebarActions?.showAdminEntry !== 'boolean') {
    pushIssue('ui.sidebarActions.showAdminEntry', 'Sidebar "show /admin/ entry" must be a boolean');
  }

  if (!isAdminSidebarDividerVariant(settings.ui?.layout?.sidebarDivider ?? '')) {
    pushIssue('ui.layout.sidebarDivider', 'Sidebar divider must be default / subtle / none');
  }

  const nav = Array.isArray(settings.shell.nav) ? settings.shell.nav : [];
  if (nav.length !== ADMIN_NAV_IDS.length) {
    pushIssue('shell.nav', 'Sidebar nav item count must match the existing nav');
  }
  ...
  nav.forEach((item, index) => {
    ...
    if (!navId) {
      ...
      pushIssue(`${basePath}.id`, `Nav item ID must not duplicate: ${navId}`);
    }
    if (navId) seenIds.add(navId);

    if (!item.label) {
      pushIssue(`${basePath}.label`, `Nav item ${item.id} display name cannot be empty`);
    }
    if (item.ornament !== null) {
      if (typeof item.ornament !== 'string') {
        pushIssue(`${basePath}.ornament`, `Nav item ${item.id} ornament must be a string or empty`);
      } else if (item.ornament.includes('\n') || item.ornament.includes('\r')) {
        pushIssue(`${basePath}.ornament`, `Nav item ${item.id} ornament must be single-line text`);
      } else if (item.ornament.length > ADMIN_NAV_ORNAMENT_MAX_LENGTH) {
        pushIssue(`${basePath}.ornament`, `Nav item ${item.id} ornament cannot exceed ${ADMIN_NAV_ORNAMENT_MAX_LENGTH} characters`);
      }
    }
    if (
      ...
      pushIssue(`${basePath}.order`, `Nav item ${item.id} order must be an integer between ${ADMIN_NAV_ORDER_MIN} and ${ADMIN_NAV_ORDER_MAX}`);
    } else if (navId && navOrderIssues.get(navId) === 'duplicate') {
      pushIssue(`${basePath}.order`, `Order must not duplicate: ${item.order}`);
    }
    if (typeof item.visible !== 'boolean') {
      pushIssue(`${basePath}.visible`, `Nav item ${item.id} visible must be a boolean`);
    }
  });
