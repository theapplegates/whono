...
const setActiveYearState = (year: number | null) => {
  ...
  if (yearMoreRoot && yearMoreTrigger) {
    ...
    yearMoreTrigger.setAttribute('aria-label', isMoreActive ? `Open more year filters, current year ${year}` : 'Open more year filters');
  }
  if (yearMoreLabel) {
...
const getEmptyResultsText = (query: string, year: number | null) => {
  ...
  return 'No matches found. Try a different keyword.';
};
