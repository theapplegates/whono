...
if (!root) {
  ...
  const getStatusPrefix = (query: string, totalMatches: number) => {
    ...
    if (query) {
      return totalMatches === 0 ? 'No matches found' : `${totalMatches} matches`;
    }
    return '';
  };
  ...
  const updateStatusForMatches = (query: string, totalMatches: number, visibleMatches: number) => {
    ...
    if (visibleMatches === totalMatches) {
      setStatus(query && !activeTagKey ? `${totalMatches} matches` : prefix);
      return;
    }
    if (visibleMatches === 0) {
      setStatus(`${prefix} (no results on this page, try the next page)`);
      return;
    }
    setStatus(`${prefix} (${visibleMatches} on this page, more on the next)`);
  };
