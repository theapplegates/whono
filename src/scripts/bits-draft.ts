...
export const initBitsDraft = (): BitsDraftController | null => {
  ...
  const setAuthorPlaceholders = () => {
    ...
    if (authorAvatarEl) {
      authorAvatarEl.placeholder = defaultAuthorAvatar
        ...
        : 'Relative image path (e.g. author/avatar.webp; leave blank for default avatar)';
    }
  };
  ...
  const readImageSize = (row: HTMLElement) => {
    ...
    image.onload = () => {
      ...
      if (!width || !height) {
        setStatus('Could not read automatically; please fill in manually.');
        return;
      }
  ...
  const collectImages = (): ImageDraft[] | null => {
    ...
    for (const row of getImageRows()) {
      ...
      if (!widthValue || !heightValue) {
        setStatus('Image URL is filled in; please add width and height.', 'error');
  ...
  const buildMarkdown = () => {
    ...
    if (!content) {
      setStatus('Please enter some content first.', 'error');
      contentEl.focus();
      return null;
    }
    ...
    if (authorAvatar === undefined) {
      setStatus('Author avatar must be a relative image path (e.g. author/avatar.webp); must not include public/, must not start with /, and must not contain URL, .., ?, #.', 'error');
